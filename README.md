# all-in-project-skill-lite（1.7.0）
...
默认采用单人串行流程；跨模块需求协调通过需求包级第 0 层拆解、委派和验收；Goal 模式通过启动时逐项确认的精确授权合同连续推进，推进中按升级分级处理用户输入——阻断级立即问、决策级攒到检查点批量问、可自主解决事项不打断。合同外写入、合并、推送、删除或历史改写仍保留明确门禁。
