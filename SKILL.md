...
普通流程中，Agent 收集证据、解释状态、推荐路线并执行已授权动作；产品取舍、技术/功能冲突、分支选择、合并策略、push、删除和历史改写必须升级给用户确认。若用户启动 Goal，则由第 0 层按当前 vault 与进度表**批量生成**一次性授权清单，用户用一次 `AskUserQuestion` 整单确认或翻 veto；合同外或冲突事项按 Goal 的升级分级处理——阻断级立即升级，决策级攒到检查点批量确认，可自主解决事项不打断用户。
